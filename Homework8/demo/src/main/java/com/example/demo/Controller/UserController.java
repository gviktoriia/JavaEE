...
public class UserController {
    ...
    public ResponseEntity register(@RequestBody final UserDTO user) {
        if (HwLibraryApplication.userService.userExists(user.getLogin())) {
        ...
        HwLibraryApplication.userService.registerUser(user);
    ...
    public ResponseEntity<List<BookDTO>> getFavorites(final Principal principal) {
        ...
        List<BookEntity> favorites = HwLibraryApplication.userService.findFavorites(login);
    ...
    public ResponseEntity addFavorites(final Principal principal, @RequestBody final BookDTO book) {
        String login = principal.getName();
        HwLibraryApplication.userService.addFavorites(BookEntity.builder().id(book.getId()).bookname(book.getName()).author(book.getAuthor()).isbn(book.getIsbn()).build(), login);
    ...
    public ResponseEntity deleteFavorites(final Principal principal, @RequestBody final BookDTO book) {
        String login = principal.getName();
        HwLibraryApplication.userService.deleteFavorites(book.getId(), login);
        return new ResponseEntity(HttpStatus.ACCEPTED);
    }
    ...
    public ResponseEntity<Boolean> isFavorite(final Principal principal, @PathVariable("id") final int id) {
        ...
        boolean fav=HwLibraryApplication.userService.isFavorite(id, principal.getName());
        return ok().body(fav);
    }
}